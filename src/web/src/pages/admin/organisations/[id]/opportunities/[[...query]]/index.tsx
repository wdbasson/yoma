...
const Opportunities: NextPageWithLayout<{
  ...
  const onSearch = useCallback(
    (query: string) => {
      if (query && query.length > 2) {
        ...
        void router.push(
          `/admin/organisations/${id}/opportunities?query=${queryEncoded}`,
        );
      } else {
        void router.push(`/admin/organisations/${id}/opportunities`);
      }
    },
    [router, id],
  );
  ...
  const handlePagerChange = useCallback(
    (value: number) => {
      // redirect
      void router.push({
        pathname: `/admin/organisations/${id}/opportunities`,
      ...
      <div className="container z-10 max-w-5xl px-2 py-8">
        <div className="flex flex-col gap-2 py-4 sm:flex-row">
          ...
          <div className="flex gap-2 sm:justify-end">
            ...
            <Link
              href={`/admin/organisations/${id}/opportunities/create`}
                ...
                <tbody>
                  {opportunities.items.map((opportunity) => (
                    <tr key={opportunity.id}>
                      <td>
                        <Link
                          href={`/admin/organisations/${id}/opportunities/${opportunity.id}/info`}
                        >
                          {opportunity.title}
                        </Link>
                      </td>
