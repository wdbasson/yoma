...
namespace Yoma.Core.Infrastructure.Database.Opportunity.Repositories
{
    public class OpportunitySkillRepository : BaseRepository<Entities.OpportunitySkill>, IRepository<OpportunitySkill>
    {
        ...
        {
            var entity = _context.OpportunitySkills.Where(o => o.Id == item.Id).SingleOrDefault();

            if (entity == null)
                throw new ArgumentOutOfRangeException(nameof(item), $"{nameof(OpportunitySkill)} with id '{item.Id}' does not exist");
