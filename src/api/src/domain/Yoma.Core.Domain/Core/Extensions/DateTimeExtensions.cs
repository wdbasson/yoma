namespace Yoma.Core.Domain.Core.Extensions
{
    public static class DateTimeExtensions
    {
        ...
        {
            return new DateTimeOffset(new DateTime(e.Year, e.Month, e.Day, 0, 0, 0, DateTimeKind.Unspecified), offSet ?? e.Offset);
        }

        public static DateTimeOffset? RemoveTime(this DateTimeOffset? e, TimeSpan? offSet = null)
        {
            if (!e.HasValue) return null;
            return new DateTimeOffset(new DateTime(e.Value.Year, e.Value.Month, e.Value.Day, 0, 0, 0, DateTimeKind.Unspecified), offSet ?? e.Value.Offset);
        }
