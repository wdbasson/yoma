...
namespace Yoma.Core.Domain.Opportunity.Services.Lookups
{
    public class OpportunityStatusService : IOpportunityStatusService
    {
        ...
        public OpportunityStatus GetByName(string name)
        {
            ...
            if (result == null)
                throw new ArgumentException($"{nameof(OpportunityStatus)} with name '{name}' does not exists", nameof(name));
        ...
        public OpportunityStatus GetById(Guid id)
        {
            ...
            if (result == null)
                throw new ArgumentException($"{nameof(OpportunityStatus)} for '{id}' does not exists", nameof(id));
