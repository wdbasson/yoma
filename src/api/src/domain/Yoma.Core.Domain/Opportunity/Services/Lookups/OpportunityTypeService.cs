...
namespace Yoma.Core.Domain.Opportunity.Services.Lookups
{
    public class OpportunityTypeService : IOpportunityTypeService
    {
        ...
        public OpportunityType GetByName(string name)
        {
            ...
            if (result == null)
                throw new ArgumentException($"{nameof(OpportunityType)} with name '{name}' does not exists", nameof(name));
        ...
        public OpportunityType GetById(Guid id)
        {
            ...
            if (result == null)
                throw new ArgumentException($"{nameof(OpportunityType)} for '{id}' does not exists", nameof(id));
